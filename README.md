...
A clean, modern desktop Weather App built with Python and Tkinter. It lets you search any city in the world and view the current conditions, an hour-by-hour forecast for today, and a full 7-day outlook — all powered by the free Open-Meteo API (no API key required).
...
- 📅 Weekly forecast with daily highs/lows and conditions
...
- **APIs:** [Open-Meteo Weather API](https://open-meteo.com/) and [Open-Meteo Geocoding API](https://open-meteo.com/en/docs/geocoding-api)
...
2. Type a city name (e.g. `Cairo`, `London`, `Tokyo`) into the **City** field.
3. Click **Search**.
4. Browse the two tabs: **Today (Hourly)** for a 24-hour forecast of the current day, or **Weekly Forecast** to pick any day from the dropdown and see the full week at a glance.
